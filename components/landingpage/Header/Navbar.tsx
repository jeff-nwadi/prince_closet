...
export default function Header() {
    ...
    return (
        <div className="fixed w-full box-border bg-[#f4f0ea] p-[10px] sm:p-[26px] z-50">
            <div className="flex justify-center font-normal relative">
                <Link href="/" className="absolute left-0 text-black no-underline heading md:text-[24px] text-[18px] ">Prince's Closet</Link>
                <div onClick={() => {setIsActive(!isActive)}} className="flex items-center justify-center gap-2 cursor-pointer">
                    ...
                    <div className="relative flex items-center">
                        <motion.p variants={opacity} animate={!isActive ? "open" : "closed"} className="m-0 text-[16px] md:text-[20px]">Menu</motion.p>
                        <motion.p variants={opacity} animate={isActive ? "open" : "closed"} className="m-0 absolute opacity-0text-[16px] md:text-[20px]">Close</motion.p>
                    </div>
                </div>
                <motion.div variants={opacity} animate={!isActive ? "open" : "closed"} className="flex gap-[30px] absolute right-0">
                    <p className="hidden sm:block sm:cursor-pointer m-0">Shop</p>
